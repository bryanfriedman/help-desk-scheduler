...
public final class SetPrefHoursAction extends Action {
  ...
  public ActionForward perform(ActionMapping mapping,
   ...
   PrefHoursForm f = (PrefHoursForm) form;
   ...
   boolean[][] weekdays = new boolean[8][5];
   for (int i = 0; i < 8; i++) {
      for (int j = 0; j < 5; j++) {
         weekdays[i][j] = false;
      }
   ...
   for (int j = 0; j < 5; j++) {
      for (int i = 0; i < 8; i++) {
         if (weekdays[i][j]) {
             sql = "INSERT INTO prefer VALUES (NULL, ";
